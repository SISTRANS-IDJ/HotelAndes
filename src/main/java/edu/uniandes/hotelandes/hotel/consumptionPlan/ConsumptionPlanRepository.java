...
public class ConsumptionPlanRepository implements ConsumptionPlanDAO {
  ...
  public int insertConsumptionPlan(ConsumptionPlan consumptionPlan) {
    final var sql =
        "INSERT INTO consumption_plan( plan_name, stay_discount, fixed_cost, plan_description)"
            + " VALUES( ?, ?, ?, ?)";
    return jdbcTemplate.update(
        sql,
        consumptionPlan.plan_name(),
        consumptionPlan.plan_discount(),
        consumptionPlan.fixed_cost(),
        consumptionPlan.plan_description());
  }

  @Override
  public Optional<ConsumptionPlan> selectConsumptionPlanById(int id) {
    final var sql =
        "SELECT id, plan_name, stay_discount, fixed_cost, plan_description FROM consumption_plan"
            + " WHERE id = ?";
  ...
  public int updateConsumptionPlan(int id, ConsumptionPlan consumptionPlan) {
    final var sql =
        "UPDATE consumption_plan SET plan_name = ?, stay_discount = ?, fixed_cost = ?,"
            + " plan_description = ? WHERE id = ?";
    return jdbcTemplate.update(
        sql,
        consumptionPlan.plan_name(),
        consumptionPlan.plan_discount(),
